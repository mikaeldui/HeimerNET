...
public static class LorDeckEncoder
{
    ...
    public static List<T> GetDeckFromCode<T>(string code) where T : IDeckCard, new()
    {
        ...
        byteQueue.Dequeue(); // remove byte for verion
        var result = new List<T>();
        for (int i = 3; i > 0; i--)
        {
            int numGroupOfs = VarintTranslator.PopVarint(byteQueue);

            for (int j = 0; j < numGroupOfs; j++)
            {
                int numOfsInThisGroup = VarintTranslator.PopVarint(byteQueue);
                var setNumber = VarintTranslator.PopVarint(byteQueue);
                var factionNumber = VarintTranslator.PopVarint(byteQueue);
                ...
                {
                    var number = VarintTranslator.PopVarint(byteQueue);
                    result.Add(new T()
                    {
        ...
        {
            int count = VarintTranslator.PopVarint(byteQueue);
            var set = VarintTranslator.PopVarint(byteQueue);
            int faction = VarintTranslator.PopVarint(byteQueue);
            var number = VarintTranslator.PopVarint(byteQueue);
            result.Add(new T()
            {
    ...
    private static void EncodeGroupOf(List<byte> bytes, List<List<IReadOnlyDeckCard>> groupOf)
    {
        ...
        for (int i = 0; i < groupOf.Count; i++)
        {
            ...
            ParseCardCode(item[0].CardCode, out var currentSetNumber, out var currentFactionCode, out var _);
